...
- 校方源码已替换成功后，旧备份清不掉只警告、不当作失败
- 课程页「去登录」不再二次确认；校方源码测试改为连真实仓库打一遍
- 校方源码同步改为先生成再替换，失败时保留上一份有效包
- 过期登录并行 401 只跳转一次登录页
- 本地协作规范改为五大原则（不向对方泄露甲 / 乙方内部与隐私信息）
- 校方源码包去掉内部称呼与预发域名，并收短库脚本注释
...
- 微信业务域名：`9c03e1f7.720roma.com`、`c73zd07zrkz.720yun.com`、`www.yunmanvr.com`
...
- 预发实测记录：`docs/测试/预发实测记录_20260901.md`（安卓真机课程/字幕/资料 + 后台直传，非双端终验）
- 课程视频 OSS PostObject 直传（预发已开；用户确认进度条 + 小程序能播 >200MB）
- 直传失败确认可「重新确认」；普通上传与直传 complete 均按 box 解析拒绝 H.265；学习资料入口对齐 AAC/M4A，列表有暂停/进度条；policy 默认 1 小时。本轮仍不做 Multipart 续传与云转码
- 8/9 号馆 VR 链接回填（2026-09-05，`sql/patch-hall-vr-8-9-20260905.sql`）：迁到已有业务域名 `2e6zb07zn85.720yun.com`，简介去掉「筹备中」，seed 已同步；十一座展馆全景齐备

### 计划中
- 真机双端补测（iOS + 4G + 登录/报名/VR/海报；媒体主链已记）
