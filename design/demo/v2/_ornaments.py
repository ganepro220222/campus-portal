...
def lico_wen():
    ...
    上一版（lico_wenpai）是我照着「門+口」手描的一个近似字，外面再套一圈牌框。
    两处都不对：
      · 手描的不是篆书，只是"看起来像"，审看意见原话是"怎么看都不舒服"；
      · 牌框把字圈起来，小尺寸下先看见一个圆角方块，字反而成了里面的噪点。

    现在直接用 design/brand/seal-script/问.svg 里那枚**真**的小篆字形——
    和问答浮标、抽屉头像、整页头像上那枚玉牌是同一个字形文件。
    一个功能一张脸，从 112rpx 的浮标到 36rpx 的列表图标都是它。

    走填充而不是描边：字形本身就是填充的闭合路径，
    描边化只会把笔画变成双线（试过，20px 下糊成一团）。
    """
    body, vb = _seal_glyph_path("\u95ee")
    vx, vy, vw, vh = [float(t) for t in vb.replace(",", " ").split()]
    ink = 26.0                                   # 墨迹最长边占 32 框的 81%
    k = ink / max(vw, vh)
    tx = 16.0 - (vx + vw / 2.0) * k
    ty = 16.0 - (vy + vh / 2.0) * k
    return _fl('<g transform="translate(%.3f %.3f) scale(%.5f)">%s</g>'
               % (tx, ty, k, body))
...
def _fl(body):
    """实心图标的外壳。线描那套是 _ln，这一枚走填充，所以另起一个。"""
    return ('<svg class="lico" viewBox="0 0 32 32" fill="currentColor" '
...
def lico_yan():
    ...
    「云中谁寄锦书来，雁字回时，月满西楼」——雁是"寄信"这件事在中文里的本相。
    纸飞机是西方符号，这套里没有它的位置。

    为什么是**实心**：描边版前后画了八版都不成立。
      · 单雁描边（颈前伸 / 双翅张 / 简笔 / 一笔 / 鸥翼加颈尾 / 侧飞）——
        颈、身、翅三件在 25px 上互相挨着，描边一粗就并成一道勾。
      · 雁阵描边（三只 V 加一竖）——V 的两端朝下、尖朝上，正是**山**的轮廓。
    实心 silhouette 没有这个问题：小尺寸下人眼认的是**外形**不是线。

    这一版按审定的参考图重画。上一版画的是"侧飞带喙的整只鸟"，
    颈、喙、双翼四件挤在一起，25px 上糊成一团勾——正是上面那条教训
    自己又犯了一遍。参考图是**正面掠过的剪影**：两道镰刀形的翼从中间
    的身分出去，上缘在当中留一个 V 形缺口，最低点是尾。只有三条边，
    再小也是一只鸟。

    几何取自参考图的量测（不是描图，是按比例重写）：
      墨迹宽高比 2.546
      左翼尖 u=0.000 v=0.284   左翼顶 u=0.161 v=0.199
      上缘缺口 u=0.501 v=0.481  右翼顶 u=0.769 v=0.000
      右翼尖 u=1.000 v=0.137   尾点   u=0.532 v=1.000
    七段三次贝塞尔按最小二乘拟合，最大偏差 0.76% 的墨迹高。
    右翼比左翼高——朝右上飞，方向本身就是"送出去"。

    画在 32 框里：宽 28（x 2→30），高 28/2.546 = 11.0（y 10.5→21.5）。
    """
    X0, W = 2.0, 28.0
    Y0, H = 10.5, 11.0
    def P(u, v):
        return '%.2f %.2f' % (X0 + W * u, Y0 + H * v)

    d = (
        # 上缘：左翼尖 → 左翼顶 → 缺口 → 右翼顶 → 右翼尖
        'M' + P(0.000, 0.284) +
        ' C' + P(0.044, 0.240) + ' ' + P(0.096, 0.210) + ' ' + P(0.161, 0.199) +
        ' C' + P(0.339, 0.198) + ' ' + P(0.419, 0.340) + ' ' + P(0.501, 0.481) +
        ' C' + P(0.545, 0.293) + ' ' + P(0.599, 0.047) + ' ' + P(0.769, 0.000) +
        ' C' + P(0.880, -0.004) + ' ' + P(0.938, 0.069) + ' ' + P(1.000, 0.137) +
        # 下缘：右翼尖 → 尾点 → 左翼中 → 左翼尖
        # （左翼这半边一段拟不住，1.74% 的偏差都在它身上；劈成两段后降到 0.76%）
        ' C' + P(0.727, 0.265) + ' ' + P(0.584, 0.639) + ' ' + P(0.532, 1.000) +
        ' C' + P(0.462, 0.843) + ' ' + P(0.385, 0.696) + ' ' + P(0.300, 0.558) +
        ' C' + P(0.215, 0.449) + ' ' + P(0.147, 0.278) + ' ' + P(0.000, 0.284) +
        ' Z'
    )
    return _fl('<path d="%s"/>' % d)
