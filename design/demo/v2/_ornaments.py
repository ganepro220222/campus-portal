...
def lico_yanzi():
    ...
    「云中谁寄锦书来，雁字回时，月满西楼」——雁字就是"寄信"这件事在
    中文里的本相：雁阵排成人字，一封信跟着飞走。
    纸飞机是西方符号，这套里没有它的位置。

    为什么是**一群**而不是一只：单画一只雁，颈、身、翅三件在 21px 上
    全糊成一道勾（试过四版，都像随手划的一笔）。雁阵反过来——
    每只只要"一个 V 加一竖"就成立，三只排成人字，小尺寸下反而读得出来，
    而且人字本身朝上，天然带方向感，正合发送。

    那一竖是雁身，不能省：只剩 V 就是三个山尖，不是雁。
    """
    def yan(cx, cy, s):
        return ('<path d="M%.2f %.2f C%.2f %.2f %.2f %.2f %.2f %.2f"/>'
                % (cx - 4.6 * s, cy + 2.9 * s, cx - 2.6 * s, cy + 1.1 * s,
                   cx - 1.2 * s, cy - 0.5 * s, cx, cy - 2.4 * s)
                + '<path d="M%.2f %.2f C%.2f %.2f %.2f %.2f %.2f %.2f"/>'
                % (cx + 4.6 * s, cy + 2.9 * s, cx + 2.6 * s, cy + 1.1 * s,
                   cx + 1.2 * s, cy - 0.5 * s, cx, cy - 2.4 * s)
                + '<path d="M%.2f %.2f L%.2f %.2f"/>'
                % (cx, cy - 2.4 * s, cx, cy + 1.4 * s))          # 雁身
    # 三只的间距和大小是量出来的：钮 42px、图标 25px 时，
    # 头雁 1.25 / 跟随 1.0 那一版三只挤在一起，糊成一小撮山尖；
    # 头雁放到 1.45、跟随放到 1.15 并把左右两只往外拉开，三只才分得开。
    return _ln(yan(16, 9.6, 1.45) + yan(7.4, 21.6, 1.15) + yan(24.6, 21.6, 1.15), 1.6)
