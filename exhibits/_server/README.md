...
`/studio-api/list` 顶层还返回 `panoramas`：供批量编辑「全景贴图」下拉使用。
候选仅来自（1）各展品 config 里声明的本地 `assets.panorama`（须为接近 2:1 的
现有文件）；（2）约定公共目录 `共享背景/`、`_panoramas/`、`shared/panoramas/` 下的
2:1 图片。不会全树扫描，避免把 2:1 模型贴图误列为全景。
...
1. **鉴权（Node 反代，staging 推荐）**：systemd 设置 `STUDIO_USER` / `STUDIO_PASS`；Nginx 只反代到本服务（见 `scripts/nginx-exhibits-editor.conf.example`），**不要**再加 `auth_basic`——浏览器只有一个 `Authorization` 头，Nginx htpasswd 与 `STUDIO_PASS` 不一致会导致 401 循环。验收脚本 `apply-staging-editor.sh` 使用同一组凭据。**PHP 静态托管**可选用 Nginx `auth_basic` 保护 HTML，但 `api.php` 仍须 `STUDIO_PASS`。公网仅部署仅观看版播放器与各展品数据。
