...
test.describe('strict WebKit startup', () => {
  ...
  test('WeChat UA downscales panorama to 2048 before PMREM by default', async ({ browser }) => {
    ...
    expect(shading.hasEnvRt).toBe(true)
    expect(shading.envRtW).toBeGreaterThan(0)
    expect(shading.configMetalness).toBeCloseTo(0.19)
    expect(shading.risks.some(r => r.id === 'pmrem-missing-metal')).toBe(false)
    expect(snap.log.some(e => e.tag === 'shading:after-pmrem')).toBe(true)
    await releaseWebGL(p)
    await p.close()
  })
