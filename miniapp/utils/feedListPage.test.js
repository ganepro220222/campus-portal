...
assert.deepStrictEqual(buildFeedLoadingPatch(FEED_LOAD.initial), {
  loading: true,
  loadingMore: false,
  error: false,
  refreshError: false
...
assert.strictEqual(pullLoad.loading, true)
...
assert.deepStrictEqual(buildFeedLoadingPatch(FEED_LOAD.loadMore), { loadingMore: true, error: false, refreshError: false })
...
assert.strictEqual(pullFail.newsList, undefined)
