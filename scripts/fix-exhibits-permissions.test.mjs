...
assert.match(sh, /--resolve api\.yunmanvr\.com:443:127\.0\.0\.1/)
assert.doesNotMatch(sh, /curl -sI http:\/\/127\.0\.0\.1\/exhibits/)
assert.doesNotMatch(sh, /chmod 755 "\$EX"/)
assert.doesNotMatch(sh, /chown root:root "\$EX"/)
assert.match(sh, /verify_content_delete_gate/)
assert.match(sh, /install_content_handoff_helper/)
assert.match(sh, /verify_studio_handoff_gate/)
assert.match(sh, /chown-exhibit-content-dir/)
assert.match(sh, /studio-exhibits-chown/)
