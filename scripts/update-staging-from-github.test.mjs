...
assert.match(sh, /wait_backend_health/, 'must retry backend health until ready')
assert.match(sh, /BACKEND_HEALTH_TIMEOUT/, 'must support configurable health wait timeout')
assert.match(sh, /backend_container_exited/, 'must fail fast when backend container exits')
