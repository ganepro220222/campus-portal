...
CREATE TABLE IF NOT EXISTS `sys_user` (
  ...
  `token_version` INT          NOT NULL DEFAULT 0 COMMENT 'JWT 版本号，改密递增',
  `create_time`   DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time`   DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `is_deleted`    TINYINT      NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `news` (
  ...
  `cover_fit_mode` VARCHAR(16) NOT NULL DEFAULT 'fill' COMMENT '封面展示：fill/fit',
...
CREATE TABLE IF NOT EXISTS `craft` (
  ...
  `poster_url`    VARCHAR(500) DEFAULT NULL COMMENT '3D加载期封面图',
  `transform_json` JSON        DEFAULT NULL COMMENT '归一化 scale/offset',
  `material_json`  JSON        DEFAULT NULL COMMENT 'PBR材质参数',
  `camera_json`    JSON        DEFAULT NULL COMMENT '初始机位',
  `viewer_enabled` TINYINT(1)  NOT NULL DEFAULT 0 COMMENT '是否开启沉浸式鉴赏',
...
CREATE TABLE IF NOT EXISTS `course` (
  ...
  `subtitle_asr_started_at` DATETIME DEFAULT NULL COMMENT 'ASR任务开始时间',
  `subtitle_asr_last_poll_at` DATETIME DEFAULT NULL COMMENT 'ASR最近轮询时间',
  `subtitle_asr_attempt_count` INT NOT NULL DEFAULT 0 COMMENT 'ASR轮询次数',
  `subtitle_asr_last_error` VARCHAR(500) DEFAULT NULL COMMENT 'ASR最近错误摘要',
  `status`           TINYINT      NOT NULL DEFAULT 1,
  `create_time`      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time`      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `is_deleted`       TINYINT      NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `subscribe_outbox` (
  `id`            BIGINT       NOT NULL AUTO_INCREMENT,
  `member_id`     BIGINT       NOT NULL,
