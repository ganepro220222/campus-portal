...
常见问法：怎么登录？怎么注册？为什么登录不了', 1, '择学号登录，输入学号与密码即可。学号登录后，也可以在个人资料页把当前微信绑定上去，之后就能免密登录。

首次登录必须修改密码。管理员导入账号时会设置初始密码，第', 500
FROM DUAL WHERE @doc_id IS NOT NULL AND @has_chunk = 0;

INSERT INTO `knowledge_chunk` (`doc_id`, `chunk_text`, `chunk_index`, `keywords`, `char_count`)
SELECT @doc_id, '出后收藏、报名等个人数据不会丢失，重新登录即可看到。

常见问法：怎么登录？怎么注册？为什么登录不了？忘记密码怎么办？怎么改密码？怎么绑定微信？怎么解绑微信？账号从哪里来？为什么提示需要绑定学号？怎么退出登录？', 2, '出后收藏、报名等个人数据不会丢失，重新登录即可看到。

常见问法：怎么登录？怎么注册？为什么登录不了？忘记密码怎么办？怎么改密码？怎么绑定微信？怎么解绑微信？账', 105
FROM DUAL WHERE @doc_id IS NOT NULL AND @has_chunk = 0;
